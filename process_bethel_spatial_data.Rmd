...
# Landmarks
ct_landmarks_pt <- landmarks(state = "CT", type = "point")
```
...
bethel_rails <- st_intersection(rails, bethel_boundary)
bethel_landmarks_pt <- st_intersection(ct_landmarks_pt, bethel_boundary)
```
...
ggplot() +
  ...
  # Rails
  geom_sf(data = bethel_rails, linetype = "twodash", size = .1) +
  
  # Landmarks
  #geom_sf(data = bethel_landmarks_pt) +
  ...
  theme_void() +
  theme(plot.caption = element_text(color = "grey10", size = 90, hjust = 0.5, family = "cormorant"))
```
